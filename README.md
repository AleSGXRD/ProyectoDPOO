...
Mi parte del trabajo consiste en la función del operador, esta consiste en la funcionalidad de la empresa ya que es el encargado de distribuir el pedido de los clientes a los taxistas en turno. Al iniciar la sesión en la parte superior izquierda muestra el nombre del usuario y debajo de este la cantidad de dinero que posee en su tarjeta de banco, más a la derecha se encuentra el botón que permite ver las secciones, a su lado se encuentra otro botón que es el que permite visualizar los turnos de trabajo de este usuario, así como añadir turnos, borrar turnos, extender el horario de estos para así poder trabajar horas extras, o para estar al pendiente y no excederse en las horas de trabajo, posterior a este  aparece el botón <trabajando> que aparece en blanco en caso de que no esté trabajando este usuario, aparece verde en su totalidad cuando el usuario está trabajando, mientras que no esté trabajando no se le mostrara ninguna petición en la pantalla, al activar esta opción le estarán apareciendo peticiones de clientes según estos la vallan solicitando. ¡Después de este botón descrito anteriormente aparece un último botón que es de las notificaciones el cual está identificado con el cierre de los signos de interrogación <!>, este te muestra el número de notificaciones en la parte inferior derecha. Además, muestra el mensaje de confirmación de si un pedido se ha realizado con satisfacción, así como el monto transferido a su cuenta, la fecha y la hora de culminado el servicio. Una vez leído el mensaje o los mensajes estos se auto eliminan.

Debajo de todos estos botones van apareciendo las solicitudes de los clientes, solo aparecen tres solicitudes como máximo, al enviar una solicitud al taxista esta desaparece de la pantalla principal y muestra otra petición en caso de que exista otra.  El operador al recibir la petición del cliente, posteriormente revisa que taxistas están en turno. La notificación llega al operador con el nombre del cliente y la dirección de este, seguidamente aparece una barra desplegable donde aparece el nombre de los taxistas donde el operador va a seleccionar a cuál de los taxistas enviar el pedido, el nombre del taxista una vez seleccionado se muestra de color rojo en caso de que este no esté trabajando, en caso de si hacerlo aparecerá de color verde en su totalidad, una vez seleccionado el taxista aparece un botón de <Enviar Petición> el cual ejecuta esta opción. Ya una vez enviada la petición solo es parte de los taxistas aceptarla o no. Cuando un taxista ya tiene un pedido y el operador le envía otra petición se muestra un mensaje de color rojo en la esquina inferior izquierda diciendo <Ya está ocupado ese taxista>, una vez que todos los taxistas estén ocupados el cliente tendrá que esperar a que alguno concluya y acepte su pedido. En caso de que algún taxista rechace una petición, esta se les enviara automáticamente a todos los taxistas que estén trabajando y no estén ocupados en otro. En caso de que todos los taxistas rechacen el pedido será notificado al cliente con un mensaje pidiéndole disculpas y con la devolución del dinero. 

En caso de que un taxista oprima la señal de <SOS> le aparecerá el botón de mensajes en rojo a los demás taxistas, diciéndole que taxi necesita su ayuda y la ubicación.

En lo que viene siendo la parte del código, primero para acceder la clase Operador hereda de la clase Usuario para que esta pueda obtener los parámetros del nombre de usuario, contraseña y el tipo de usuario, y así poder acceder a esta cuenta. En caso de que no coincidan alguno de estos parámetros se mostrara el mensaje de que la contraseña o usuario son incorrectos. A la hora de asignar un pedido, se crean dos <if> uno dentro del otro, el primero para saber si el taxista seleccionado se encuentra trabajando, entonces con el <else> retorno el mensaje <No está trabajando>, mediante que en el segundo <if> se comprueba si este taxista no tiene ningún pedido, que al igual con su correspondiente <else> retorna el mensaje <Ya está ocupado ese taxista>. Cuando el pedido es negado o no por todos los taxistas, para esto creo una variable de tipo booleana inicializada en un principio falsa, después creo un <for> con dos <if> dentro que el primero es para ver que taxista se encuentra trabajando y el segundo es para ver que taxistas de esos que están trabajando están disponibles, y el <for> cumple el objetivo de que se realice las funciones de los <if> hasta que no quede ningún taxista. Después de cerrar el <for> creo otro <if> para si es cierto que no hay disponibilidad muestre el mensaje al cliente. 

Cuando el pedido ha sido cumplido utilizo una serie de <try><catch> para recargarle el 20% del dinero cobrado al taxista, el 10% al operador y el 70% a la empresa. Todo esto es notificado a los respectivos usuarios mediante un mensaje, incluyendo al cliente. 

Para cuando un taxista oprime el botón de <SOS> cree un <for> dentro del método llamado de esa misma forma donde contiene un <if> dentro de este mismo <for> para que verifique todos los taxistas que están de servicio y enviarle la señal de <SOS> a todos.

Para activar y desactivar el botón de <trabajando> cree un <if> para que si el botón es seleccionado pase al color verde y me pase a trabajar con el <for> de adentro que es el que me permite ir recorriendo todos los pedidos de los clientes, pero dentro de este <for> cree un <if> para que a medida que valla enviándole las peticiones a los taxistas me valla incluyendo los otros pedidos pendientes que no se muestran en la pantalla ya que le di como máximo 3 pedidos a mostrar, ya cuando termine todo este bucle si no hay más pedidos que mostrar, es decir que son menos de 3 o ninguno la pantalla se encontrara totalmente en blanco, solo mostrando el nombre del operador y la cantidad de dinero abajo, además de los botones de <Ver Sesiones> , <Ver Turnos> , <Trabajando> y por ultimo y no menos importante el de los mensajes.

Entonces utilice el <else> para si no es seleccionado el botón de trabajar no me aparezcan los pedidos de los clientes. En fin, solo se mostrarán los pedidos al activar el botón
